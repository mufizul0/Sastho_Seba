...
const AmbulenceReservation = () => {
  const { user } = useAuth();
  const [morder, setMorder] = useState([]);
  const [filteredData, setFIlteredData] = useState([]);
  
  useEffect(() => {
    fetch("https://api-sastho-seba.onrender.com/am/rsv/data")
      .then((res) => res.json())
      .then((data) => {
        setMorder(data);
        setFIlteredData(morder?.filter((mor) => mor.amb_name === user.displayName));
      });
  }, [morder, user.email]);
  return (
    <div className="">
          ...
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr className="h-16 w-full text-sm leading-none text-gray-800">
                ...
                <th className="font-bold text-left pl-12">Ambulence Contact</th>
                <th className="font-bold text-left pl-16">Reservation Date</th>
                <th className="font-bold text-left pl-16">Status</th>
...
export default AmbulenceReservation;

const Card = ({ props }) => {
  const [show, setShow] = useState(null);
  const handleDelete = (id) => {
    axios.delete(`https://api-sastho-seba.onrender.com/am/rsv/data/${id}`);
  }
  return (
    <>
      <tr className="h-20 text-sm leading-none text-gray-800 bg-white hover:bg-gray-100 border-b border-t border-gray-100">
        ...
        <td className="pl-12">
          <p className="font-medium">{props.amv_name}</p>
          <p className="text-xs leading-3 text-gray-600 mt-2">{props.amb_cont}</p>
        </td>
